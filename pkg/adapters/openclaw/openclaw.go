...
// non-container OpenClaw version. It is opt-in and is executed without a shell.
...
func (a *Adapter) runtimeVersion(ctx context.Context) (string, []string) {
	if container := a.dockerContainer(); container != "" {
		...
		if err != nil {
			return "", []string{"openclaw version discovery failed: " + err.Error()}
		}
		return version, nil
	}
	...
	if err != nil {
		return "", []string{"openclaw version command failed: " + err.Error()}
	}
	return version, nil
}
...
func (a *Adapter) configuredVersionCommand() string {
	...
	return strings.TrimSpace(getenv("SIDEPLANE_OPENCLAW_VERSION_COMMAND"))
}
